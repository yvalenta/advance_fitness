...
- 2026-08-31: **DESPLEGADO con GO ("push y despliega con kamal")**: pushes hechos (app `28b3f95`, raíz `cdfc591`), y `kamal deploy` verde en 120,6s — build remoto en el propio Lightsail, contenedor nuevo healthy con el sha exacto, drain de 130s del viejo, hook post-deploy ok. Verificado en producción: `db:migrate:status` → las dos migraciones de puestos **up**, backfill **36 puestos = 36 usuarios** con tenant, puerta pública en su 302. Tropiezo del camino: net-ssh moría con el em-dash del host de panelita en ~/.ssh/config (US-ASCII) — se corre Kamal con locale UTF-8, sin tocar el archivo de otra sesión. El standby quedó descubierto AUSENTE (murió con el disco): imagen ya transferida (1.05GB) y tarea propia creada.
